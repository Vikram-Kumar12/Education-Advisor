...
const CollegeDetails = ({ college, onClose }) => {
  const [activeTab, setActiveTab] = useState('courses');
  const [expandedCourse, setExpandedCourse] = useState(null);

  // Sample course data for the college
  const coursesData = [
    {
      id: 1,
      title: `${college.courses[0]}`,
      duration: "4 Years",
      seats: 120,
      fees: college.fees,
      description: "Focuses on fundamental concepts with modern technologies and industry-relevant skills.",
      highlights: college.highlights,
      eligibility: "10+2 with 75% aggregate and relevant entrance exam qualified",
      placement: college.placement
    },
    {
      id: 2,
      title: `${college.courses[1] || college.courses[0]}`,
      duration: "4 Years",
      seats: 90,
      fees: college.fees,
      description: "Covers advanced concepts and specialized knowledge in the field.",
      highlights: college.highlights,
      eligibility: "10+2 with 75% aggregate and relevant entrance exam qualified",
      placement: college.placement
    }
  ...
  return (
    <div className="min-h-screen  text-white ">
      ...
      <motion.div 
        ...
        transition={{ duration: 0.7 }}
      >
        <img src={college.image} alt="college" className="object-cover w-full h-full rounded-md" />
        <div className="absolute rounded-md top-0 left-0 w-full h-full bg-gradient-to-r from-black to-orange-900 bg-opacity-80 flex items-center justify-center">
          <div className="text-center text-white px-4">
            <motion.h1 
              className="text-4xl md:text-5xl font-bold mb-2"
              initial={{ y: 30, opacity: 0 }}
              ...
              >
                    ...
                    <div className="space-y-4">
                      {college.courses.map((course, index) => (
                        ...
                        >
                          <div className="flex justify-between items-start mb-3">
                            <div>
                              <h4 className="text-xl font-bold text-white">{course}</h4>
                              <div className="flex flex-wrap gap-2 mt-2">
                                <span className="bg-orange-900 text-orange-200 text-xs px-3 py-1 rounded-full">4 Years</span>
                                <span className="bg-orange-900 text-orange-200 text-xs px-3 py-1 rounded-full">120 Seats</span>
                                <span className="bg-orange-900 text-orange-200 text-xs px-3 py-1 rounded-full">{college.fees}</span>
                              </div>
                            </div>
                            <button
                              ...
                              className="text-orange-500 hover:text-orange-400 font-medium"
                            >
                              {expandedCourse === index ? 'Show Less' : 'View Details'}
                            </button>
                          </div>
                          
                          <p className="text-gray-300 mb-3">Comprehensive program covering fundamental and advanced concepts in the field.</p>
                          
                          <div className="flex flex-wrap gap-2 mb-3">
                            {college.highlights.slice(0, 3).map((skill, i) => (
                              <span key={i} className="bg-gray-700 text-gray-300 text-xs px-3 py-1 rounded-full">{skill}</span>
                            ))}
                            ...
                            {expandedCourse === index && (
                              ...
                              >
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                  <div>
                                    <h5 className="font-semibold text-orange-400 mb-2">Eligibility</h5>
                                    <p className="text-sm text-gray-300">10+2 with 75% aggregate and relevant entrance exam qualified</p>
                                  </div>
                                  <div>
                                    <h5 className="font-semibold text-orange-400 mb-2">Placement</h5>
                                    <p className="text-sm text-gray-300">{college.placement}</p>
                                  </div>
                                </div>
                                <div className="mt-4">
                                  <h5 className="font-semibold text-orange-400 mb-2">Specializations</h5>
                                  <div className="flex flex-wrap gap-2">
                                    {college.highlights.map((skill, i) => (
                                      <span key={i} className="bg-orange-900 text-orange-200 text-xs px-3 py-1 rounded-full">{skill}</span>
                                    ))}
                                  </div>
                                </div>
                ...
                {activeTab === "cutoff" && (
                  <div>
                    ...
                    <div className="bg-orange-900 bg-opacity-30 rounded-xl p-5 mb-6 border border-orange-800">
                      <h4 className="text-lg font-semibold mb-2 text-orange-300">General Eligibility Criteria</h4>
                      <ul className="list-disc list-inside text-gray-300 space-y-1">
                        <li>Minimum 75% aggregate in 10+2 examination with relevant subjects</li>
                        <li>Valid entrance exam score (varies by course)</li>
                        <li>Age limit: 17-25 years</li>
                        <li>Indian nationals only</li>
                      </ul>
                    </div>
                        ...
                        >
                          ...
                          <div className="mt-2 flex items-center">
                            <div className="w-full bg-gray-700 rounded-full h-2.5">
                              <div 
                                className="bg-gradient-to-r from-orange-500 to-orange-700 h-2.5 rounded-full" 
                                style={{ width: `${parseFloat(college.cutoff)}%` }}
                              ></div>
                            </div>
                            <span className="ml-2 text-sm font-bold text-orange-400">{college.cutoff}</span>
                          </div>
                        </motion.div>
                      ))}
                    </div>
                ...
                {activeTab === "facilities" && (
                  <div>
                    <h3 className="text-2xl font-bold mb-6 text-orange-400 border-b border-orange-800 pb-2">Campus Facilities</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {college.highlights.map((facility, index) => (
                        <motion.div 
                          key={index}
                          className="bg-gray-800 rounded-xl p-5 border border-gray-700"
                          initial={{ opacity: 0, y: 20 }}
                        ...
                        >
                          <div className="flex items-start">
                            <span className="text-3xl mr-4 text-orange-400">⭐</span>
                            <div>
                              <h4 className="text-lg font-semibold text-white mb-2">{facility}</h4>
                              <p className="text-gray-300">State-of-the-art facility with modern equipment and resources.</p>
                            </div>
                          </div>
                        </motion.div>
                      ))}
